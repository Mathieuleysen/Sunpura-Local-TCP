...
class AECCBatteryClient:
    ...
    async def _set(self, command: str, extra: Optional[Dict] = None) -> Optional[Dict[str, Any]]:
        ...
        The battery stores register values without reliably sending an ack,
        so waiting for one only blocks the GET poll and makes entities
        go unavailable. We send and move on.
        """
        payload: Dict[str, Any] = {
            "Set": command,
            "SerialNumber": self._next_serial(),
